...
func Delete_time_entry_rates_time_entry_rate_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_time_entry_rates_time_entry_rate_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_time_entry_rates_time_entry_rate_id",
		mcp.WithDescription("Delete time entry rate"),
		mcp.WithString("time_entry_rate_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_time_entry_rates_time_entry_rate_idHandler(cfg),
	}
}
