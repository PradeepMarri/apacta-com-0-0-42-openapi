...
func Get_mass_messages_users_mass_messages_user_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_mass_messages_users_mass_messages_user_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_mass_messages_users_mass_messages_user_id",
		mcp.WithDescription("View mass message"),
		mcp.WithString("mass_messages_user_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_mass_messages_users_mass_messages_user_idHandler(cfg),
	}
}
