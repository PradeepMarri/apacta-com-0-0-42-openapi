...
func Delete_companies_vendors_companies_vendor_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_companies_vendors_companies_vendor_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_companies_vendors_companies_vendor_id",
		mcp.WithDescription("Delete a companies vendor"),
		mcp.WithString("companies_vendor_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_companies_vendors_companies_vendor_idHandler(cfg),
	}
}
