...
func Get_forms_form_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_forms_form_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_forms_form_id",
		mcp.WithDescription("View form"),
		mcp.WithString("form_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_forms_form_idHandler(cfg),
	}
}
