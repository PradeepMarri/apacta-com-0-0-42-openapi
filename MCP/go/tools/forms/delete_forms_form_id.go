...
func Delete_forms_form_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_forms_form_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_forms_form_id",
		mcp.WithDescription("Delete a form"),
		mcp.WithString("form_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_forms_form_idHandler(cfg),
	}
}
