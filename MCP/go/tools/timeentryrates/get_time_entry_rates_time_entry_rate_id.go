...
func Get_time_entry_rates_time_entry_rate_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_time_entry_rates_time_entry_rate_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_time_entry_rates_time_entry_rate_id",
		mcp.WithDescription("View time entry rate"),
		mcp.WithString("time_entry_rate_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_time_entry_rates_time_entry_rate_idHandler(cfg),
	}
}
