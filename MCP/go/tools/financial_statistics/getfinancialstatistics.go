...
func GetfinancialstatisticsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["only_not_invoiced"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("only_not_invoiced=%v", val))
		}
		...
		url := fmt.Sprintf("%s/financial_statistics%s", cfg.BaseURL, queryString)
...
func CreateGetfinancialstatisticsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_financial_statistics",
		mcp.WithDescription("Get general statistics"),
		mcp.WithString("date_from", mcp.Description("")),
		mcp.WithString("date_to", mcp.Description("")),
		mcp.WithString("project_id", mcp.Description("")),
		mcp.WithString("project_status_ids[]", mcp.Description("")),
		mcp.WithBoolean("only_not_invoiced", mcp.Description("")),
		mcp.WithBoolean("details", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetfinancialstatisticsHandler(cfg),
	}
}
