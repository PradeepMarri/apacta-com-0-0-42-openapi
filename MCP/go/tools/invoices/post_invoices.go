...
func Post_invoicesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/invoices", cfg.BaseURL)
...
func CreatePost_invoicesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_invoices",
		mcp.WithDescription("Add invoice"),
		...
		mcp.WithString("created_or_modified_gte", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_invoicesHandler(cfg),
	}
}
