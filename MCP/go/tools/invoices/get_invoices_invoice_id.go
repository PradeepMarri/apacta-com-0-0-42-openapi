...
func Get_invoices_invoice_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_invoices_invoice_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_invoices_invoice_id",
		mcp.WithDescription("View invoice"),
		mcp.WithString("invoice_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_invoices_invoice_idHandler(cfg),
	}
}
